...
Game::Game(sf::RenderWindow& win) : window(win), player1(), player2(), player1Turn(true), animationRunning(false), frameIndex(0), diceValue(0) {
    ...
    rollButton.setSize(sf::Vector2f(100, 50)); // Button size
    rollButton.setFillColor(sf::Color::Blue);  // Button color
    rollButton.setPosition(270, 500);          // Button position (adjust if needed)
    ...
    rollButtonText.setPosition(280, 510); // Adjust for proper alignment inside the button
...
void Game::rollDice() {
    ...
    while (animationRunning) {
        ...
        if (animationClock.getElapsedTime().asMilliseconds() > 50) {
            ...
            drawText(player1Turn ? "Player 1's Turn" : "Player 2's Turn", 50, 20, sf::Color::Yellow);
...
void Game::handleInput() {
    ...
    if (sf::Mouse::isButtonPressed(sf::Mouse::Left)) {
        ...
        // Determine the current player and grid offsets
        int offsetX = player1Turn ? 50 : 450;  // Current player's grid start
        int column = -1;
        int row = -1;

        // Check if the click is on the current player's grid
        if (mousePos.x >= offsetX && mousePos.x < offsetX + 3 * 70 &&
            mousePos.y >= 200 && mousePos.y < 200 + 3 * 70) {
            column = (mousePos.x - offsetX) / 70; // Calculate column index
            row = (mousePos.y - 200) / 70;       // Calculate row index
            std::cout << "Mouse clicked at: " << mousePos.x << ", " << mousePos.y << std::endl;
            std::cout << "Column: " << column << ", Row: " << row << std::endl;
            updateGrid(player1Turn ? player1 : player2, column, row); // Pass both column and row
        }
    }
}
...
void Game::updateGrid(Player& player, int column, int row) {
    ...
    // If it's Player 2's turn, remove matching dice from Player 1
    if (!player1Turn) {
        int removedRow = player1.removeOpponentDice(row, column, diceValue);
        if (removedRow != -1) {
            std::cout << "Player 2 removed Player 1's die from row " << removedRow + 1
...
void Game::draw() {
    ...
    // Highlight safe columns
    auto safeColumns = getSafeColumnsForCombos(player1Turn ? player2 : player1);
    for (int col : safeColumns) {
        sf::RectangleShape highlight(sf::Vector2f(60, 180)); // Height of 3 cells
        highlight.setPosition((player1Turn ? 450 : 50) + col * 70, 200);
